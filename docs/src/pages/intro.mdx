---
title: HarmonyLite - Introduction
---

# What & Why?

HarmonyLite is a distributed SQLite replicator with leaderless, and eventual consistency. It allows you to build a robust replication
...
SQLite is probably the most ubiquitous DB that exists almost everywhere, HarmonyLite aims to make it even more ubiquitous for server
...
Download [latest](https://github.com/wongfei2009/harmonylite/releases/latest) HarmonyLite and extract package using:

```
tar vxzf harmonylite-v*.tar.gz
```

From extracted directory run `examples/run-cluster.sh`. Make a change in `/tmp/harmonylite-1.db` using:

```
bash > sqlite3 /tmp/harmonylite-1.db
sqlite3 > INSERT INTO Books (title, author, publication_year) VALUES ('Pride and Prejudice', 'Jane Austen', 1813);
```

Now observe changes getting propagated to other database `/tmp/harmonylite-2.db`:

```
bash > sqlite3 /tmp/harmonylite-2.db
...
HarmonyLite is essentially a CDC (Change Data Capture) and replication pipeline running top of NATS. It can automatically configure appropriate
...
HarmonyLite on the other hand is born different. It's born to act as a side-car to your existing processes:
 - Instead of requiring single primary, there is no primary! Which means any node can make changes to its local DB.
   HarmonyLite will use triggers to capture your changes (hence atomic records), and then stream them off to NATS.
...
In HarmonyLite every row is uniquely mapped to a JetStream. This guarantees that for any node to publish changes for a row it
...
Ask harmonylite to remove hooks and log tables by:
...
### Can I use HarmonyLite as single primary and multiple replicas?
