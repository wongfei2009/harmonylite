...
There are multiple components in HarmonyLite that make it work. Let's look at them one by one: 


## Triggers and data capture

HarmonyLite works by using a pretty basic trick so that each process that's access database can capture changes,
and then HarmonyLite can publish them to rest of the nodes. This is how it works internally:

- Global table `__harmonylite___global_change_log` that captures exact sequence of operations committed to DB.
  ...
  columns in the table. So each table gets a `__harmonylite__<table_name>_change_log` that will record
...
When you are running HarmonyLite process, it's watching for changes on DB file and WAL file. Everytime there is a change
HarmonyLite:
...
As of `v0.6.0+` HarmonyLite supports taking snapshot and being able to fully restore it via
[NATS Object Storage](https://docs.nats.io/using-nats/developer/develop_jetstream/object).

 > In future HarmonyLite plans to support more storage mechanisms like S3 (and compatible APIs including BlackBlaze
...
After which HarmonyLite removes all the hooks, and triggers from the snapshot, and re-VACUUM packs the database for upload.
Once done this snapshot is uploaded to `OBJ_<stream-prefix>-snapshot-store` (`<stream-prefix>-snapshot-store` in
client API).

One important thing to keep in mind is everytime HarmonyLite sees a sequence number in a shard higher than what it
...
NATs server as specified in section above. This snapshot is downloaded in a temporary path. HarmonyLite then
uses `exclusive` transaction lock to prevent any writers from getting into DB while it copies over
the files.

Once snapshot has been copied over, HarmonyLite installs same triggers, and change log tables again in DB, and starts
processing logs applying them all on the snapshot. This means while HarmonyLite is restoring the DB it's quite
